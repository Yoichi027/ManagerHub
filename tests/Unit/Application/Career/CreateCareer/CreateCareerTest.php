...
final class CreateCareerTest extends \Codeception\Test\Unit
{
    public function testCreatesCareerAndFirstEmptySeasonInOneTransaction(): void
    {
        ...
        $careers = new InMemoryCareers();
        $seasons = new InMemorySeasons();
...
final class InMemoryCareers implements CareerRepository
{ /** @var list<Career> */ public array $items = [];
    public function findById(\Ramsey\Uuid\UuidInterface $id): ?Career
    {
        return null;
...
final class InMemorySeasons implements SeasonRepository
{ /** @var list<Season> */ public array $items = [];
    public function findById(\Ramsey\Uuid\UuidInterface $id): ?Season
    {
        return null;
    } public function findActiveByCareerId(\Ramsey\Uuid\UuidInterface $id): ?Season
    {
        return null;
...
final readonly class SingleClubRepository implements ClubRepository
{
    public function __construct(private Club $club) {} public function findById(\Ramsey\Uuid\UuidInterface $id): ?Club
    {
        return $id->equals($this->club->id) ? $this->club : null;
    } public function all(): array
    {
        return [$this->club];
    }
}
final readonly class SingleLeagueRepository implements LeagueRepository
{
    public function __construct(private League $league) {} public function findById(\Ramsey\Uuid\UuidInterface $id): ?League
    {
        return $id->equals($this->league->id) ? $this->league : null;
    } public function all(): array
    {
        return [$this->league];
    }
