...
final readonly class MysqlCareerRepository implements CareerRepository
{
    ...
    {
        $row = $this->connection->select('*')->from('careers')->where(['id' => $id->toString(), 'is_deleted' => false])->one();
    ...
    {
        $this->connection->createCommand()->insert('careers', $this->values($career))->execute();
    }
    public function save(Career $career): void
    {
        $this->connection->createCommand()->update('careers', $this->values($career), ['id' => $career->id->toString()])->execute();
    }
