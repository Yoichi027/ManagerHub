...
<section class="account-page">
    ...
    <div class="account-grid">
        ...
        <section class="account-section"><div><h2>Password</h2><p>Choose a strong password you do not use elsewhere.</p></div>
            <form id="account-password-form" method="post" action="<?= Html::encode($urlGenerator->generate('account.password')) ?>" novalidate><?= $csrf->hiddenInput()->render() ?><label class="form-label" for="current-password">Current password</label><div class="password-field"><input class="form-control" id="current-password" name="current_password" type="password" autocomplete="current-password" required><button class="password-toggle" type="button" data-password-toggle aria-controls="current-password" aria-pressed="false">Show</button></div><label class="form-label" for="new-password">New password</label><div class="password-field"><input class="form-control" id="new-password" name="new_password" type="password" autocomplete="new-password" minlength="8" required><button class="password-toggle" type="button" data-password-toggle aria-controls="new-password" aria-pressed="false">Show</button></div><div class="form-text">8–128 characters, with an uppercase letter, number and symbol.</div><?php if ($passwordError !== null): ?><div class="invalid-feedback d-block" role="alert"><?= Html::encode($passwordError) ?></div><?php endif ?><button class="button button--primary" type="submit">Update password</button></form>
        </section>
        <section class="account-section account-section--danger"><div><h2>Deactivate account</h2><p>Your career data is preserved, but you will be signed out and unable to log in.</p></div>
            <form id="account-deactivate-form" method="post" action="<?= Html::encode($urlGenerator->generate('account.deactivate')) ?>" novalidate><?= $csrf->hiddenInput()->render() ?><label class="form-label" for="deactivate-password">Confirm your password</label><div class="password-field"><input class="form-control" id="deactivate-password" name="password" type="password" autocomplete="current-password" required><button class="password-toggle" type="button" data-password-toggle aria-controls="deactivate-password" aria-pressed="false">Show</button></div><?php if ($deactivateError !== null): ?><div class="invalid-feedback d-block" role="alert"><?= Html::encode($deactivateError) ?></div><?php endif ?><button class="button button--danger" type="submit">Deactivate account</button></form>
        </section>
    </div>
</section>
